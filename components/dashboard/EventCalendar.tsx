        ...
        >
          <CarouselContent className="flex h-full">
            {dataCarousel.map((data, index) => (
              <CarouselItem
                key={index}
                className="md:basis-1/2 lg:basis-1/3 h-full w-2 flex justify-center"
              >
                <EventCard
                  title={data.title}
          ...
          <CarouselPrevious className="hidden p-4 sm:flex justify-center items-center absolute top-[-100px] right-76" />
          <CarouselNext className="hidden sm:flex justify-center items-center absolute top-[-100px] right-2" />
        </Carousel>
      </div>
    </div>
  );

...
const SubmissionDetailModal: React.FC<SubmissionDetailModalProps> = ({ submission, isOpen, onClose, onActionSuccess }) => {
  ...
  const handleUpdateStatus = async (status: 'accepted' | 'rejected') => {
    ...
    const { error } = await supabase
      .from('berkas')
      .update({ status: status, rejection_reason: status === 'rejected' ? rejectionReason : null })
      .eq('id', submission.id);
    ...
    if (error) {
      ...
      alert(`Berkas berhasil di-${status === 'accepted' ? 'terima' : 'tolak'}.`);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl p-6 sm:p-8 relative animate-fade-in-up">
        ...
        <h3 className="text-xl font-bold text-gray-800 mb-4">Review Berkas: {submission.profiles?.full_name}</h3>
        
        <div className="space-y-3 text-sm border-t border-b py-4 mb-4">
          <p><strong>NIM:</strong> {submission.profiles?.nim}</p>
          <p><strong>Jenis Laporan:</strong> {submission.jenis_laporan}</p>
          <p><strong>Tanggal Unggah:</strong> {new Date(submission.created_at).toLocaleString('id-ID')}</p>
          <p><strong>Status Saat Ini:</strong> {submission.status}</p>
          {submission.status === 'rejected' && submission.rejection_reason && (
             <p><strong>Alasan Ditolak Sebelumnya:</strong> {submission.rejection_reason}</p>
          )}
          <a href={submission.file_url} target="_blank" rel="noopener noreferrer" className="inline-block mt-2 bg-blue-600 text-white font-semibold py-2 px-4 rounded-md hover:bg-blue-700 transition">
